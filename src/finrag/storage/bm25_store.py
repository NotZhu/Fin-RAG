...
class PostgreSQLBM25StateStore:
    ...
    def _ensure_schema(self) -> None:
        ...
        with self.db.connect() as conn: # 连接数据库并获取连接对象
            ...
            execute(conn, "CREATE INDEX IF NOT EXISTS idx_bm25_chunks_document ON bm25_chunks(document_id)")
    ...
    def replace_document_chunks(self, document_id: str, chunk_token_counts: Dict[str, Dict[str, int]]) -> None:
        ...
        with self.db.connect() as conn: # 连接数据库并获取连接对象
            ...
            for chunk_id, token_counts in chunk_token_counts.items():
                ...
                execute(
                    conn,
                    "INSERT INTO bm25_chunks (chunk_id, document_id, token_count) VALUES (%s, %s, %s)",
                    (chunk_id, document_id, sum(clean_counts.values())),
                )
                # 插入该分块的所有词项词频记录
                for term, count in clean_counts.items():
                    term_id = self._ensure_term(conn, term)
                    affected_term_ids.add(term_id)
                    # 插入该分块的词项词频记录 到 BM25 词频表
                    execute(
                        conn,
                        "INSERT INTO bm25_term_counts (chunk_id, term_id, count) VALUES (%s, %s, %s)",
                        (chunk_id, term_id, count),
                    )
            # 刷新词项文档频率
            self._refresh_term_dfs(conn, affected_term_ids)
    ...
    def delete_document(self, document_id: str) -> None:
        ...
        with self.db.connect() as conn: # 连接数据库并获取连接对象
            ...
            rows = execute(
                ...
                JOIN bm25_chunks bc ON bc.chunk_id = btc.chunk_id
                WHERE bc.document_id = %s
                """,
                (document_id,),
            ).fetchall()
            ...
            execute(
                ...
                WHERE chunk_id IN (SELECT chunk_id FROM bm25_chunks WHERE document_id = %s)
                """,
                (document_id,),
            )
            # 删除指定文档的所有分块记录
            execute(
                conn,
                "DELETE FROM bm25_chunks WHERE document_id = %s",
                (document_id,)
            )
            # 刷新词项文档频率
            self._refresh_term_dfs(conn, affected_term_ids)
    ...
    def clear(self) -> None:
        ...
        """
        with self.db.connect() as conn: # 连接数据库并获取连接对象
            # 直接清空分块和词频表，并将词项表中的文档频率重置为 0
            execute(conn, "DELETE FROM bm25_term_counts")
            execute(conn, "DELETE FROM bm25_chunks")
            execute(conn, "UPDATE bm25_terms SET df = 0")
    ...
    def build_query_sparse_vector(self, tokens: Iterable[str]) -> SparseVector:
        ...
        with self.db.connect() as conn:
            rows = execute(
                conn,
                f"SELECT term_id, term FROM bm25_terms WHERE term IN ({','.join(['%s'] * len(token_counts))})",
                tuple(token_counts.keys()),
            ).fetchall()
    ...
    def build_document_sparse_vector(self, tokens: Iterable[str]) -> SparseVector:
        ...
        with self.db.connect() as conn:
            total_chunks, avgdl = self._collection_stats(conn)
            rows = execute(
                conn,
                f"SELECT term_id, term, df FROM bm25_terms WHERE term IN ({','.join(['%s'] * len(token_counts))})",
                tuple(token_counts.keys()),
            ).fetchall()
    ...
    @staticmethod
    def _collection_stats(conn: Any) -> tuple[int, float]:
        """
        读取 BM25 分块集合统计信息
        Args:
            conn: 当前数据库连接
        Returns:
            分块数量和平均 token 数
        """
        row = execute(conn, "SELECT COUNT(*), COALESCE(AVG(token_count), 0) FROM bm25_chunks").fetchone()
        return int(row[0] or 0), float(row[1] or 0.0)

    def _ensure_term(self, conn: Any, term: str) -> int:
        """
        确保词项存在并返回 term_id
        Args:
            conn: 当前数据库连接
        ...
        """
        # 检查该词项是否已存在
        row = execute(conn, "SELECT term_id FROM bm25_terms WHERE term = %s", (term,)).fetchone()
        # 如果该词项已存在，直接返回其 ID
        if row is not None:
            return int(row[0])
        # 如果该词项不存在，插入新记录
        execute(conn, "INSERT INTO bm25_terms (term, df) VALUES (%s, 0)", (term,))
        # 返回新插入的词项 ID
        row = execute(conn, "SELECT term_id FROM bm25_terms WHERE term = %s", (term,)).fetchone()
        return int(row[0])

    def _refresh_term_dfs(self, conn: Any, term_ids: Iterable[int]) -> None:
        """
        刷新指定词项的文档频率
        Args:
            conn: 当前数据库连接
        ...
        for term_id in set(int(term_id) for term_id in term_ids):
            ...
            row = execute(
                conn,
                "SELECT COUNT(DISTINCT chunk_id) FROM bm25_term_counts WHERE term_id = %s",
                (term_id,),
            ).fetchone()
            # 更新该词项的文档频率
            execute(conn, "UPDATE bm25_terms SET df = %s WHERE term_id = %s", (int(row[0] or 0), term_id))
