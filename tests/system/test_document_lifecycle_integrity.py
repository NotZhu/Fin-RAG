...
def test_indexing_updates_bm25_state_for_each_leaf_chunk_before_refresh(tmp_path):
    class RecordingBM25Store:
        def __init__(self):
            self.replaced = {}
            self.deleted = []
    ...
    bm25_store = RecordingBM25Store()
    system.bm25_store = bm25_store
...
def test_vector_upsert_failure_rolls_back_new_bm25_state(monkeypatch, tmp_path):
    class RecordingBM25Store:
        def __init__(self):
            self.replaced = []
            self.deleted = []
    ...
    def fail_upsert(*args, **kwargs):
        raise RuntimeError("Milvus 写入失败")

    monkeypatch.setattr(IndexConstructionModule, "upsert_leaf_nodes", fail_upsert)
